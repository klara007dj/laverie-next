...
export async function GET(req: NextRequest) {
  try {
    ...
    if (role === 'ADMIN') {
      const [user, washes, clients, stations, services, stats] = await Promise.all([
        prisma.user.findUnique({ where: { id: userId } }),
        prisma.wash.findMany({ include: { service: true, station: true, user: true }, orderBy: { createdAt: 'desc' } }),
      ...
      const totalWashes   = washes.filter(w => w.statut === 'COMPLETED').length
    ...
    const [user, washes, vouchers] = await Promise.all([
      prisma.user.findUnique({ where: { id: userId } }),
      prisma.wash.findMany({ where: { userId }, include: { service: true, station: true, review: true }, orderBy: { createdAt: 'desc' }, take: 10 }),
    ...
    return NextResponse.json({ user, washes, vouchers, totalWashes, totalReservations, loyalty })
  } catch (e) {
